...
AActor* UObjectPoolSubsystem::GetObjectFromPool(TSubclassOf<AActor> ClassPool)
{
	if (!GetWorld())
	{
		return nullptr;
...
void UObjectPoolSubsystem::ReturnObjectToPool(TSubclassOf<AActor> ClassPool, AActor* ActorToReturn)
{
	if (!GetWorld())
	{
		return;
